...
{
    public class AwsCognitoAuthProvider : IOAuth2Provider
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly ILogger<AwsCognitoAuthProvider> _logger;

        public string ProviderName => "Cognito";

        public AwsCognitoAuthProvider(IHttpClientFactory httpClientFactory, ILogger<AwsCognitoAuthProvider> logger)
        {
        ...
        public string BuildAuthorizationUrl(ExternalAuthProvider config, string redirectUri, string state)
        {
            ...
            {
                throw new InvalidOperationException("Cognito domain is required");
            }
        ...
        public async Task<OAuth2TokenResponseDto> ExchangeCodeForTokenAsync(ExternalAuthProvider config, string code, string redirectUri)
        {
            ...
            var domain = config.Domain.TrimEnd('/');
            var tokenEndpoint = $"https://{domain}/oauth2/token";
        ...
        public async Task<ExternalUserInfoDto> GetUserInfoAsync(ExternalAuthProvider config, string accessToken)
        {
            ...
            var response = await client.GetAsync($"https://{domain}/oauth2/userInfo");
            ...
            return new ExternalUserInfoDto
            {
                ...
                DisplayName = userInfo.TryGetProperty("name", out var name) ? name.GetString() : 
                             userInfo.TryGetProperty("username", out var username) ? username.GetString() : null
            };
        }
        ...
        public async Task<bool> RevokeTokenAsync(ExternalAuthProvider config, string token)
        {
            try
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Post, $"https://{domain}/oauth2/revoke")
                {
                ...
                {
                    _logger.LogInformation("Successfully revoked Cognito token");
                    return true;
                }
                
                _logger.LogWarning("Failed to revoke Cognito token: {StatusCode}", response.StatusCode);
                return false;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error revoking Cognito token");
                return false;
            }
        }
        ...
        private async Task<OAuth2TokenResponseDto> PostTokenRequestAsync(string tokenEndpoint, Dictionary<string, string> requestBody)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogError("Cognito token request failed: {StatusCode} - {Content}", response.StatusCode, content);
